...
public class Query_v2 {
	...
	private static Double alpha_1 = new Double(0.4);
	private static Double alpha_2 = new Double(0.4);
	...
	public static void main(String args[]) throws Exception {
		...
		String articleIndex = "C:/Users/Purusharth/Desktop/Ranking/DocIndex";
		String passageIndex = "C:/Users/Purusharth/Desktop/Ranking/SentenceIndex";
		...
		//Seperate init for baseline queries
		...
		String path_stopwords = "C:/Users/Purusharth/Desktop/IBM/Selected_Results/Selected_Queries/stopwords.txt";
		...
		String query_path = "C:/Users/Purusharth/Desktop/IBM/Selected_Results/q5.txt";
		...
		for(int i=0;i<query_list.size();i++){
			...
			//System.out.println("INDRI:"+indri_queryList.get(i));
			//System.out.println("NEW:"+query_string);
		...
		FileWriter fw1 = new FileWriter("C:/Users/Purusharth/Desktop/IBM/Selected_Results/Selected_Queries/processed_q5.txt");
	    	 ...
	    	 public void run() {
	    		 try{
	    	        for(int i=0;i<baseline_processed_queryList.size();i++){
	    	        	System.out.println("Performing baseline on:**********************");
	    	    		System.out.println(baseline_processed_queryList.get(i));
	...
	private static void non_BaseLineQuery(QueryEnvironment passage_env, QueryEnvironment article_env, String query, String indri_query) throws Exception{
		...
		System.out.println("Performing proposed on:^^^^^^^^^^^^^^^^^^^^^^^^^^");
		System.out.println(query);
		
		//George H. W. Bush medical condition disease disability disorder health problem ailment illness paralympic Parkinson's disease
				//String query = "George H. W. Bush medical condition disease disability disorder health problem ailment illness paralympic Parkinson disease";
				...
				result = passage_env.runQuery(myQuery, 96686802);
				...
				for(int i=0;i<docVec.length;i++) {
					...
					for(int j=0;j<positions.length;j++) {
						...
						//System.out.println("Doc:"+i+" Word:"+stems[positions[j]] +" at position:"+(j-1));
				...
				FileWriter fw = new FileWriter("C:/Users/Purusharth/Desktop/IBM/Selected_Results/Results/Q5/New/"+query.replaceAll(" ","_")+".txt");
				BufferedWriter bw = new BufferedWriter(fw);
				PrintWriter out = new PrintWriter(bw);
				
				for(int i=0;i<ranked_set.size();i++){
					String text = ranked_set.get(i).getPassage();
		...
		try{
			...
			for(int i=0;i<passage_docs.size();i++){
				...
				for(int j=0;j<query_terms.size();j++){
					...
					long collection_count = countTerm(env,query_entity);
					...
					doc_score += (double)(alpha_2*(article_count+1))/(doc_vocab_size+article_length);
	...
	private static ArrayList<Entity_v2> populateParentCounts(QueryEnvironment passage_env, QueryEnvironment env, ArrayList<Entity_v2> passage_docs, int[] docIds){
		try{
			...
			ArrayList<Integer> bad_ids = new ArrayList<>();
			...
			for(int i=0;i<passages.length;i++) {
				...
				ScoredExtentResult[] query_results = env.runQuery(field_query,10000);
				...
				if(query_results!=null && query_results.length!=0){
					...
					for(int j=0;j<article_docVec.length;j++) {
						...
						passage_docs.get(i).setParent_docLength(parent_docLength);
